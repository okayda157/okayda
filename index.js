...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=+5561998083846:+55 (61) 998083846\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐘𝐚𝐞 𝐌𝐞𝐮 𝐏𝐢𝐭 @${num.split('@')[0]}\nBem vindo (𝐀) ao grupo *${mdata.subject}* LEIA AS REGRAS PARA NN SER REMOVIDO POHA`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ PACIÊNCIA POW SE N FOR FAZ DNV⌛',
				success: '✔️Sucesso✔️',
				...
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nSLV mano como!\nVocê nn está registrado no banco de dados Do OKAYDA tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Okayda|16`,
				}
			}
			...
			const ownerNumber = ["5561998083846@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5561998083846@s.whatsapp.net'
                        /******ApiKey Input******/
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MARCANDO GERAL QUEM NN GOSTA QUE SE FODA😋🤙 〙✪══\n╠➥'+teks+'╚═〘 𝐎𝐊𝐀𝐘𝐃𝐀 𝐁𝐎𝐓 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 MARCANDO GERAL QUEM NN GOSTA QUE SE FODA😋🤙 〙✪══\n╠➥'+teks+'╚═〘 𝐎𝐊𝐀𝐘𝐃𝐀 𝐁𝐎𝐓〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 MARCANDO GERAL QUEM NN GOSTA QUE SE FODA😋🤙 〙✪══\n╠➥'+teks+'╚═〘 𝐎𝐊𝐀𝐘𝐃𝐀 𝐁𝐎𝐓〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					reply('╔══✪〘 MARCANDO GERAL QUEM NN GOSTA QUE SE FODA😋🤙 〙✪══\n╠➥'+teks+'╚═〘 𝐎𝐊𝐀𝐘𝐃𝐀 𝐁𝐎𝐓 〙')
					break
				case 'send':
					var pc = body.slice(6)
				...
				if (!isQuotedImage) return reply(`Envie fotos com legendas ${prefix}setbotpp ou tags de imagem que já foram enviadas`)
					...
					reply('Obrigado pelo novo perfil seu viado😗')
					break
				case 'bc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Tchau Vai Nem Fazer Falta FDP👋', text) // ur cods
					}, 0)
                     break
                ...
                break
                   ...
                   break
					...
					break
                                  case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('TU JÁ SE REGISTRO POW🐦🤙')
